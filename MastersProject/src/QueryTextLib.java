...
public class QueryTextLib { 
	...
	private void handleEquation(String equation) {
		...
		String conditions = splitTwo[1];
			...
			ArrayList<ArrayList<int[]>> myTbl = colConstr(joinTable, joinCols, colBuilder);
	...
	//New function to handle variables in constraints
	private ArrayList<ArrayList<int []>> colConstr(ArrayList<ArrayList<int []>> table, ArrayList<String> colms, ArrayList<String> colConstr){
		ArrayList<ArrayList<int []>> originalTbl = table;
		ArrayList<String> keepToProj = colms;
		...
		for(String s : colConstr) {
			...
			if(equal) {
				...
				ArrayList<int[]> ogVars = new ArrayList<>();
				boolean unique = true;
				for (ArrayList<int []> a : originalTbl) {
					int[] value = a.get(colNo);
					unique=true;
					for(int[] b : ogVars) {
						if((b[0]==value[0])&&(b[1]==value[1])) {
							unique=false;
						}
					}
					if(unique) {
				...
				for(int n=0;n<conditions.length();n++) {
					columns.add(conditions.substring(n, n+1));
				}

				ArrayList<ArrayList <int[]>> compilation = new ArrayList<>();
				for(int[] a : ogVars) {
					ArrayList<ArrayList <int[]>> allRecords = new ArrayList<>();
					ArrayList<ArrayList <int[]>> theseRecords = makeWholeTbl(a[0], a[1], conditions.length());
					ArrayList <int[]> singleRecord = new ArrayList<>();
					int[] entry = new int[2];
					...
					compilation.addAll(allRecords);

				}

				compilation=eliminateRepititionsInTbl(compilation);

				compilation=duplicateCols(compilation, columns);

				ArrayList<Integer> freshCols = new ArrayList<>();
				for(int count=0; count<columns.size();count++) {
					boolean found = false;
					for(int m=0; m<freshCols.size(); m++) {
						if(columns.get(freshCols.get(m)).equals(columns.get(count))){
							found=true;
						}
					}
					if(!found) {
				...
				compilation = removeDuplicColsFromTbl(compilation, freshCols);

				ArrayList<String> cols = new ArrayList<>();
				for(int m : freshCols) {
					cols.add(columns.get(m));
				}


				originalTbl = join(originalTbl, colms, compilation, cols);
				colms = colJoin(colms, cols);
			}else {
				String first = split[0];
				String conditions = split[1];
				int colNo = -1;
				...
				ArrayList<ArrayList<int[]>> other = join(originalTbl, colms, compilation, cols);
				ArrayList<ArrayList<int[]>> newtb = new ArrayList<>();
				
				
				for(ArrayList<int[]> d : originalTbl) {
					boolean found = false;
					for(ArrayList<int[]> p : other) {
						boolean match=true;
						for(int u=0; u<keepToProj.size();u++) {
							int[] r = d.get(u);
							int[] t = p.get(u);
							if(!((r[0]==t[0])&&(r[1]==t[1]))) {
								match=false;
		...
		ArrayList<ArrayList<int[]>> finalTbl = projection(originalTbl, colms, keepToProj);
	...
	//Redundant method to apply column constraints to tables
		...
		for(ArrayList<int[]> a : table) {
			...
			for(String s : colConstr) {
				...
				for(int j=0; j<conditions.length();j++) {
					...
					int[] intToCheck = new int[2];
					if(colNo==-1) {
						intToCheck[0]=0;
						intToCheck[1]=0;
					}else {
						intToCheck = a.get(colNo);
					}
	...
	public ArrayList<Integer> SuffixArray (){
			...
			for(int i=0;i<word.length();i++) {
				subst=word.substring(i);
				arrayOfUnsortedSuffixes.add(subst);
			}
	...
	//Creates the suffix array in O(nlogn) time and solves memory issue
	//Original code can be found at https://leetcode.com/problems/longest-duplicate-substring/discuss/929600/java-onlogn-132-ms-faster-than-94-kasai-algorithm
	 public ArrayList<Integer> suffixArray() {
		 ArrayList<Integer> suffixArray = new ArrayList<Integer>();
		 	String S = STOREDWORD;
		    int n = STOREDWORDLENGTH;
		    Integer[] rank = new Integer[n];
		    for (int i = 0; i < n; i++)
		      rank[i] = n - i - 1;

		    Arrays.sort(rank, (a, b) -> Character.compare(S.charAt(a), S.charAt(b)));
		    ...
		    suffixArray.add(STOREDWORDLENGTH);
		    for(int a : sa) {
		    	suffixArray.add(a);
		    }
		    return suffixArray;
	...
	private ArrayList<Integer> trialGenerateLCPArray() {
		...
		for(int i=word.length()-1; i>=0;i--) {
			...
			if(invSuff == 0) {
				...
				priorValue = suffixArray.get(invSuff-1);
	...
	public ArrayList<String> enumerateSubwordsOfSetLength(int len) {
		ArrayList<String> subwordList = new ArrayList<String>();
		if(len<0) {
			System.out.println("Please enter a valid subword length");
		}else if(len==0) {
			subwordList.add(" ");
		}else {
			for(int i=1; i< SUFFIXARRAY.size(); i++) {
				if(SUFFIXARRAY.get(i)+len <= STOREDWORDLENGTH) {
					int lcp = LCPARRAY.get(i);
					if(lcp < len) {
						String strToAdd =  STOREDWORD.substring(SUFFIXARRAY.get(i), SUFFIXARRAY.get(i)+len);
						subwordList.add(strToAdd);
					}
				}
			}
		}
	...
	//Returns number of subwords of the inputted length
	public int noOfSubwordsOfSetLength(int len) {
		int noOfSubwords = 0;
		if(len<0) {
			System.out.println("Please enter a valid subword length");
		}else if(len==0) {
	...
	//Returns the number of maximum possible non-unique subwords from the text length
	public long maxSubwords() {
		long n = STOREDWORDLENGTH;
		long result = ((n*(n+1))/2)+1;
		return result;
	}
	
	
	//Returns the total number of subwords of the text
	public long noOfSubwords() {
		long sb = 0;
		int count = 0;
		sb=sb+1;
	...
	//Public method that returns an ArrayList of Integer arrays
	public ArrayList<int []> enumerateAllSubwordsReturningPos() {
			ArrayList<int []> subwordList = new ArrayList<int[]>();
			int [] subword = new int[2];
			int count = 0;
			subword[0]=STOREDWORDLENGTH;
	...
	private ArrayList<ArrayList<int []>> projection(ArrayList<ArrayList<int []>> oldTable, ArrayList<String> oldCols, ArrayList<String> newCols) {
	...
	private ArrayList<ArrayList<int []>> antijoin(ArrayList<ArrayList<int []>> tableOne, ArrayList<String> colsOne, ArrayList<ArrayList<int []>> tableTwo, ArrayList<String> colsTwo) {
	...
	//Method to join two tables
	private ArrayList<ArrayList<int []>> join(ArrayList<ArrayList<int []>> tableOne, ArrayList<String> colsOne, ArrayList<ArrayList<int []>> tableTwo, ArrayList<String> colsTwo) {
	...
	private ArrayList<ArrayList<int []>> semijoin(ArrayList<ArrayList<int []>> tableOne, ArrayList<String> colsOne, ArrayList<ArrayList<int []>> tableTwo, ArrayList<String> colsTwo) {
		ArrayList<ArrayList<int []>> newTbl = new ArrayList<>();
		ArrayList<Integer> colIntMatch = new ArrayList<>();
